...
class order_controller extends general_controller
{
    public function action_confirm()
    {
        ...
        if($cart)
        {
            ...
            $payment_method_list = vcache::instance()->payment_method_model('indexed_list');
            foreach($payment_method_list as $key => $val){
                
                $payment_method_list[$key]['banklist'] = get_banklist($val['bank_list']);
                unset($payment_method_list[$key]['bank_list']);
            }
    ...
    public function action_submit()
    {
        ...
        if($order_model->create($data))
        {
            ...
            jump(url('pay', 'index', array('order_id' => $data['order_id'],'banklist' => $banklist)));
        }
        else
        {
