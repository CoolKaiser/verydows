<?php
class banner_controller extends general_controller
{
    public function action_index()
    {
        if(request('step') == 'search')
        {
            ...
            $total = $banner_model->query("SELECT COUNT(*) as count FROM {$banner_model->table_name} {$where}", $binds);
            if($total[0]['count'] > 0)
            {
                $limit = $banner_model->set_limit(array(request('page', 1), request('pernum', 15)), $total[0]['count']);
                $sql = "SELECT banner_id,name, picture, start_date, end_date, seq, status
                        FROM {$banner_model->table_name} {$where}
                        ORDER BY adv_id DESC {$limit}
                       ";              
                $list = $banner_model->query($sql ,$binds);
                ...
                //     $v['type'] = $banner_model->type_map[$v['type']];
    ...
    public function action_add()
    {
        ...
        else
        {
            ...
            $this->compiler('adv/add_banner.html');
        }
    }
